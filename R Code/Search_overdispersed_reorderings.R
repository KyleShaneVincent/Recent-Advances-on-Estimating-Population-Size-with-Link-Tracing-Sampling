#The "Search_overdispersed_reorderings" function will conduct a search for overdispersed sample reorderings to use in the Gelman_Rubin function.
#The first searches for one smaller in probability of selection relative to the original reordering.
#The second searches for one larger in probability of selection relative to the original reordering.

Search_overdispersed_reorderings <- function(s, search_iterations, use_beta_est, var_est, var_est_new)
	{
	...
	P.prob.list.1 = P.prob.list.start			
	
	accept_search_1 = accept_search_2 = numeric(search_iterations)	
	s.resample = s	
	for(k in 1:search_iterations)
		{
		Resample_selection(s.resample=s.resample, P.prob.list=P.prob.list, beta_strata_calc=beta_strata_calc, beta_sex_worker_calc=beta_sex_worker_calc, 
							beta_net_size_calc=beta_net_size_calc)
		if(Q.prob > log(0))
			if(Q.prob < P.prob.list.1[[cluster_select]])
				{
				P.prob.list.1[[cluster_select]] = Q.prob
				s.resample = s.resample.proposed
				accept_search_1[k] = 1
				}
		}
	P.prob.overdispersed.1 <<- sum(unlist(P.prob.list.1))
	s.overdispersed.1 <<- s.resample
	accept_search_1 <<- accept_search_1

	P.prob.list.2 = P.prob.list.start
	s.resample = s	
	for(k in 1:search_iterations)
		{
		Resample_selection(s.resample=s.resample, P.prob.list=P.prob.list, beta_strata_calc=beta_strata_calc, beta_sex_worker_calc=beta_sex_worker_calc, 
							beta_net_size_calc=beta_net_size_calc)
		if(Q.prob > P.prob.list.2[[cluster_select]])
			{
			P.prob.list.2[[cluster_select]] = Q.prob
			s.resample = s.resample.proposed
			accept_search_2[k] = 1
			}
		}
	P.prob.overdispersed.2 <<- sum(unlist(P.prob.list.2))
	s.overdispersed.2 <<- s.resample
	accept_search_2 <<- accept_search_2
	
	cat("The log probability of the original reordering is", sum(unlist(P.prob.list.start)), '\n')
	cat('\n')
	
	cat("The log probabilities of the overdispersed reorderings are", P.prob.overdispersed.1, P.prob.overdispersed.2, '\n')
	cat('\n')
	
	cat("The total number of \"accepted\" overdispersed reorderings in the first and second search algorithms are, respectively,", 
			sum(accept_search_1), "and", sum(accept_search_2), '\n')
	cat('\n')			
			
	cat("The original estimates corresponding with these reorderings (and which will act as MCMC chain seeds) are, respectively,", '\n')
	cat("First reordering:", '\n')
	FS_estimate(s0=s.overdispersed.1$Wave_0, var_est) 
	cat("Estimator based on linkage counts", '\n')
	print(N.hat.strata)
	cat("Estimator based on node counts", '\n')
	print(N.hat.strata.1)
	cat("Second reordering:", '\n')
	FS_estimate(s0=s.overdispersed.2$Wave_0, var_est_new) 
	cat("Estimator based on linkage counts", '\n')
	print(N.hat.strata)
	cat("Estimator based on node counts", '\n')
	print(N.hat.strata.1)
	...
	cat("First reordering:", '\n')
	FS_new_estimate(s0=s.overdispersed.1$Wave_0, var_est) 
	cat("Estimator based on linkage counts", '\n')
	print(N.hat.strata.new)
	cat("Estimator based on node counts", '\n')
	print(N.hat.strata.1.new)
	cat("Second reordering:", '\n')
	FS_new_estimate(s0=s.overdispersed.2$Wave_0, var_est_new) 
	cat("Estimator based on linkage counts", '\n')
	print(N.hat.strata.new)
	cat("Estimator based on node counts", '\n')
	print(N.hat.strata.1.new)	
