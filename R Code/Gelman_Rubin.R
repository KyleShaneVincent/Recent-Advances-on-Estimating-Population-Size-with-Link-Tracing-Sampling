#The "Gelman_Rubin" function will apply the Gelman-Rubin test for convergence. MCMC chains commence with "overdispersed" seed sample reorderings.

Gelman_Rubin <- function(s.overdispersed.1, s.overdispersed.2, use_beta_est, var_est, var_est_new)
	{
	print("Working on first part...")
	MCMC(s=s.overdispersed.1, num_chains=1, search_iterations=0, use_beta_est=use_beta_est, var_est=var_est, var_est_new=var_est_new)
	GR_N.hat.strata_1 = mcmc(N.hat.strata.mcmc) 
	GR_N.hat.strata.new_1 = mcmc(N.hat.strata.new.mcmc)
	GR_N.hat.strata.1_1 = mcmc(N.hat.strata.1.mcmc) 
	GR_N.hat.strata.1.new_1 = mcmc(N.hat.strata.1.new.mcmc)
	print("Mean acceptance rate for first chain is:")
	print(mean(accept_mcmc))	

	print("Working on second part...")
	MCMC(s=s.overdispersed.2, num_chains=1, search_iterations=0, use_beta_est=use_beta_est, var_est=var_est, var_est_new=var_est_new)
	GR_N.hat.strata_2 = mcmc(N.hat.strata.mcmc) 
	GR_N.hat.strata.new_2 = mcmc(N.hat.strata.new.mcmc)
	GR_N.hat.strata.1_2 = mcmc(N.hat.strata.1.mcmc) 
	GR_N.hat.strata.1.new_2 = mcmc(N.hat.strata.1.new.mcmc)
	print("Mean acceptance rate for second chain is:")
	print(mean(accept_mcmc))
	...
	for(k in 1:num_strata)
		{
		GR_N.hat.strata[[k]] = mcmc.list(GR_N.hat.strata_1[,k], GR_N.hat.strata_2[,k])
		GR_N.hat.strata.new[[k]] = mcmc.list(GR_N.hat.strata.new_1[,k], GR_N.hat.strata.new_2[,k])
		GR_N.hat.strata.1[[k]] = mcmc.list(GR_N.hat.strata.1_1[,k], GR_N.hat.strata.1_2[,k])
		GR_N.hat.strata.1.new[[k]] = mcmc.list(GR_N.hat.strata.1.new_1[,k], GR_N.hat.strata.1.new_2[,k])
		}
	...
	for(k in 1:num_strata)
		...
		xlab="MCMC Iteration (first overdispersed reording in blue and second in red)", ylab="Strata Pop Size", col="blue", ylim=range(c(GR_N.hat.strata[[k]][[1]], GR_N.hat.strata[[k]][[2]])))
	...
	for(k in 1:num_strata)
		...
		xlab="MCMC Iteration (first overdispersed reording in blue and second in red)", ylab="Strata Pop Size", col="blue", ylim=range(c(GR_N.hat.strata.1[[k]][[1]], GR_N.hat.strata.1[[k]][[2]])))
	...
	for(k in 1:num_strata)
		...
		xlab="MCMC Iteration (first overdispersed reording in blue and second in red)", ylab="Strata Pop Size", col="blue", ylim=range(c(GR_N.hat.strata.new[[k]][[1]], GR_N.hat.strata.new[[k]][[2]])))
	...
	for(k in 1:num_strata)
		...
		xlab="MCMC Iteration (first overdispersed reording in blue and second in red)", ylab="Strata Pop Size", col="blue", ylim=range(c(GR_N.hat.strata.1.new[[k]][[1]], GR_N.hat.strata.1.new[[k]][[2]])))
